...
class ZLinuxMemory[N <: JnaNative] {
  ...
  protected def failZIO: ZIO[ZEnv, IOException, IOException] = ZIO.fail(new IOException("Error"))
}
...
object ZLinuxMemory {
  ...
  def make: ZManaged[ZEnv, IOException, ZLinuxMemory[Native]] =
    ZManaged.make(
      ZIO.effect {
        val zLinuxMemory = new ZLinuxMemory[Native]
        zLinuxMemory.load
        zLinuxMemory
      } mapError( ex =>
        if (!Platform.isLinux)
